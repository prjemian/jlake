 ...
 *
 */
public class ExtrapolateFunction implements ExtrapolateInterface {

	HashMap<String, ExtrapolateFunctionBase> knownForms;
	ExtrapolateFunctionBase functionalForm;
	String name;
	 ...
	 * I(Q) = constant
	 */
	public ExtrapolateFunction() {
		// I don't like this idea but it works.  Looks fragile.
		knownForms = new HashMap<String, ExtrapolateFunctionBase>();
		// TODO Can instancing be avoided until a key is identified?
		knownForms.put("flat background",	new ExtrapolateConstant());
		knownForms.put("linear",			new ExtrapolateLinear());
		knownForms.put("power law",			new ExtrapolatePower());
		knownForms.put("Porod law", 		new ExtrapolatePorod());
		// knownForms.put("power law + flat",	new ExtrapolatePowerPlusFlat());

		select("flat background");
	}
	
	public void select(String str) {
		if (knownForms.containsKey(str)) {
			name = str;
			functionalForm = knownForms.get(str);
		} else {
			functionalForm = null;
			name = "";
		}
	...
	public void fitData(double[] Qsas, double[] Isas, double[] Idev, double qStart) {
		functionalForm.fitData(Qsas, Isas, Idev, qStart);
	}
	...
	public double valueOf(double qNow) {
		return functionalForm.valueOf(qNow);
	}

	/**
	 * @return String representing fitted equation: I(Q) = ...
	 */
	public String toString() {
		return functionalForm.toString();
	}

	/**
	 * report the names (keys) of the extrapolation available functions
	 * @return
	 */
	public Set<String> availableFunctions() {
		return knownForms.keySet();
	}

	/**
	 * @return the name of the extrapolation function
	 */
	...
	public static void main(String[] args) {
		...
		ExtrapolateFunction fit = new ExtrapolateFunction();

		for (String form : fit.availableFunctions()) {
			fit.select(form);
			System.out.printf("#----------------------------\n%s:\n", fit.getName());
			fit.fitData(Qsas, Isas, Idev, 0.1);
